...
int main(int argc, char *argv[]) {
    ...
    while (running) {
        ...
        { std::lock_guard<std::mutex> lk(locationInfo.mtx); loc_copy = locationInfo; }

        heat_add_points(loc_copy);

        std::map<int, PciHistory> hist_copy;
        { std::lock_guard<std::mutex> lk(history_mtx); hist_copy = pci_histories; }
